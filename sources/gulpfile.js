...
function scssBlocks(cb) {
  gulp
    .src(['./blocks/**/*.scss', '!./blocks/__example/**'], { allowEmpty: true, sourcemaps: true })
    .pipe(plumber({ errorHandler: onError }))
    .pipe(cached('scss'))
    .pipe(dependents())
    .pipe(
...
function scssBlocksRelease(cb) {
  gulp
    .src(['./blocks/**/*.scss', '!./blocks/__example/**'], { allowEmpty: true })
    .pipe(plumber({ errorHandler: onError }))
    .pipe(sass.sync({ style: 'compressed' }))
    .pipe(autoprefixer())
...
function blocksFiles(cb) {
  gulp
    .src(['./blocks/**/block.json', './blocks/**/render.php', '!./blocks/__example/**'], { allowEmpty: true })
    .pipe(gulp.dest('../blocks'));
  cb();
}
...
function watch(cb) {
  ...
  gulp.watch('./blocks/**/*.scss', gulp.series(scssBlocks)).on('unlink', (filepath) => {
    delete cached.caches.scss[filepath];
  });
